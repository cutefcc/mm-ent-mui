...
//   const html = md.render(fm.body);
  ...
  // 获取用户设置的文件名或者制作新的文件名
  const filename = this.data.remainingRequest.slice(
    this.data.remainingRequest.lastIndexOf("/") + 1
  );

  const url = path.resolve(__dirname, `../src/webapp/md_trans/${filename}.js`);
  fs.writeFileSync(url, resultHtml);
//   callback(null, resultHtml);
  // or 使用 emitFile 输出文件
  // const url = path.join(`../src/md_trans/${filename}.js`);
